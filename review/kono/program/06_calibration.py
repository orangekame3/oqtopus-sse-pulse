...
interval = 100 * 1000 * 10  # 測定から初期化までの待機時間(ns単位)
# interval = 100 * 1000  # 測定から初期化までの待機時間(ns単位)
# max_execution_time = 20 * 60  # 最大実行時間（秒）

try:
    time_before_calibration = time.time()
    ...
    time_after_calibration = time.time()
    print(f"calibration time: {time_after_calibration - time_before_calibration} seconds")

    classifiers = ex.measurement._classifiers

    time_after_generate_classifiers = time.time()
    print(f"classifier generation time: {time_after_generate_classifiers - time_after_calibration} seconds")
    ...
    #     if time_before_measurement - time_before_calibration > max_execution_time:
    #         break
    ...
    #     # 結果を整形してJSON形式で出力
    #     result = {
    #         "start_time": datetime.datetime.fromtimestamp(time_before_measurement).strftime('%Y-%m-%d %H:%M:%S'),
